...
# Was going to manually put it in, but I got the above code to work fine.
# kable::kbl(tibble( ` ` = c("thinning", "initial size", "thinning:initial_size", "residuals"),
#         Df = c(1, 1, 1, 7),
#         `Sum Sq` = c(590, 4846, 11, 748),
#         `Mean Sq` = c(590, 4846, 11, 107),
#         `F value` = c(5.5, 45.4, 0.1, NA),
#         `Pr(>F)` = c(0.05, 0, 0.75, NA)) )
...
car::Anova(myaov, type = 3, contrast = list(factorA = contr.sum, factorB = contr.sum))
```
...
-   `contrast` argument tells R how to parameterize the X matrix. In other words, R actually changes a factor variable into a series of columns to represent the various levels. This tells R what values should go in those columns.
