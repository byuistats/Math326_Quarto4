...
$$y_\text{ijk} = \mu + \alpha_i + \beta_j + (\alpha\beta)_\text{ij} + \epsilon_\text{ijk}$$ {#eq-bf2}
...
::: callout-important
In a completely randomized factorial design, each experimental unit is randomly assigned to exactly 1 factor level combination (aka treatment).
:::
...
It can be helpful to first list out all the subjects, factor level combinations, and the order in a non-randomized table. This can be done by listing all the subjects, then listing the values of each factor in a separate column.

::: column-margin
Subjects are the experimental units in this example. The terms are used interchangeably for these factorial designs.
:::
...
paste <- rep(rep(c("name", "off"),each=3), times = 4)
...
Now randomize the treatments assignment to subject with the `sample` function. You can also randomize the order column.

```{r}
#| code-fold: true

set.seed(17) #This is so that others can replicate your work and start with the same random seed.

#Randomize the treatment assignment to subject
randomized_table <- assignment_table %>% 
  mutate(Subject = sample(Subject)) %>% 
  mutate(order = sample(order)) #you can also randomize the order of experimental runs if needed
...
$$y_\text{ijk} = \mu + \alpha_i + \beta_j + (\alpha\beta)_\text{ij} + \epsilon_\text{ijk}$$ {#eq-bf2_2}
...
All other residuals were similarly obtained. Ultimately, @fig-factor-effects displays all the factor level effects that are summed to obtain each observation.
...
$$SS_\text{total} = 365.6 + 344.5 + 654.3 + ... + 452.4 + 548.2  = 12,674.30$${#eq-ss-obs}
...
$$SS_\text{toothbrush} = 6*(0.11) + 6*(7.77) + 6*(0.01) + 6*(6.50) = 86.31$${#eq-ss-int} <br>

The sum of squares is similarly calculated for toothpaste brand (@eq-ss-paste) and brush by paste interaction (@eq-ss-int).

$$SS_\text{toothpaste} = 12*(0.03) + 12*(0.03) = 0.62$${#eq-ss-paste}
...
$$SS_\text{residual} = 18.35 + 0.84 + 8.72 + ... + 2.11 + 1.03  = 143.82$${#eq-ss-resid}
...
[Levene's test](assumptions.html#constant-variance) can provide insight on whether the trend in the residual plot is drastic enough to constitute a violation of the constant variance assumption.
