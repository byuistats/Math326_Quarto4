...
::: callout-note
## Contrasts of Means vs. Contrasts of Effects

Though we have been working with the effects model up to this point, this page about contrasts is presented in terms of means because they are easier to talk about and to conceptualize. In reality, contrasts of means are identical to contrasts of effects and will always yield the same result. For example, look at the comparison of the means of the oscilatting brush to the manual brush.

$\mu_\text{osc} - \mu_\text{man} = 0$

This can be rewritten as

$(\mu + \alpha_\text{osc}) - (\mu + \alpha_\text{man}) = 0$

where $\mu$ without a subscript is the grand mean. The expression can be simplified further.

$\mu + \alpha_\text{osc} - \mu - \alpha_\text{man} = 0$

$\mu - \mu + \alpha_\text{osc} - \alpha_\text{man} = 0$

$\alpha_\text{osc} - \alpha_\text{man} = 0$

Thus, effects can be swapped in place of means and vice versa in all of these contrast equations
:::
...
$$t = \frac{(\bar{y}_\text{man} - \bar{y}_\text{osc})}{\sqrt{s^2_p*(\frac{1}{n_1} + \frac{1}{n_2})}}$$ {#eq-diff2means}
...
$$y_\text{ij} = \mu + \alpha_i + \epsilon_\text{ij}$$ {#eq-model1}
...
$$t = \frac{ `r round(mean_man,2) - round(mean_osc,2)` }{\sqrt{ `r round(mse, 2)` * (\frac{1}{ `r nsize` } + \frac{1}{ `r nsize` })}} = `r round(tstat,2)`$$ {#eq-tvalue}
...
$$t = \frac{\hat{\psi}}{s_\hat{\psi}} = \frac{\hat{\psi}}{\sqrt{MSE * \sum(c^2_i / n_i)}}$$ {#eq-contrast}
...
Numerator: $$\hat{\psi} = 1 * `r round(means_tbl[[1,2]],2)` + -\frac{1}{3} * `r round(means_tbl[[2,2]],2)` + -\frac{1}{3} * `r round(means_tbl[[3,2]],2)` + -\frac{1}{3} * `r round(means_tbl[[4,2]],2)` = 0.438$$

Denominator: $$s_\hat{\psi} = \sqrt{MSE * \sum(c^2_j / n_j)} = \sqrt{ `r round(mse, 2)` * \left( \frac{1^2}{6} + \frac{-\frac{1}{3}^2}{6} + \frac{-\frac{1}{3}^2}{6} + \frac{-\frac{1}{3}^2}{6} \right) } = 1.294$$

t statistic: $$t = \frac{\hat{\psi}}{s_\hat{\psi}} = \frac{0.438}{1.294} = .34$$ {#eq-contrast4}
...
: First Pair of Contrasts Showing Orthogonal Contrasts
...
: Comparing a Different Set of Contrasts that Are NOT Orthogonal Contrasts
...
: Orthogonality of Contrasts from @tbl-coefficients
