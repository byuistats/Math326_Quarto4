...
$$t = \frac{ `r round(mean_man,2)` - `r round(mean_osc,2)` }{\sqrt{ `r round(mse, 2)` * (\frac{1}{ `r nsize` } + \frac{1}{ `r nsize` })}} = `r round(tstat,2)`$$ {#eq-tvalue}
...
| All pairs of contrasts from @tbl-coefficients |   | Orthogonality |
|-------------------------|-----------------------------|------------------|
