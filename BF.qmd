...
::: column-page
![Example of BF1 Factor Structure](images/BF1_structure_equation.JPG)
:::
...
In the lifeboat example, **benchmark** has one level (shown by the one cell in @fig-bf1-structure-with-equation) and there are no outside factors for benchmark. Therefore, the degrees of freedom for benchmark is one.
...
For **treatment factor**, there are four levels of the factor (shown by the four vertically long cells for treatment in @fig-bf1-structure-with-equation). Benchmark is the only factor outside of treatment. Take the number of levels for treatment factor (4 training methods) and subtract the degrees of freedom for benchmark (1), which yields 4-1 = **3 degrees of freedom** for treatment.
...
For **residual errors**, there are 24 levels of the factor (as shown by the 24 cells for residual error on the far right of @fig-bf1-structure-with-equation). Both benchmark and treatment are outside factors for the residual errors factor. Take the number of levels for the residual error (24) and subtract the sum of the degrees of freedom for benchmark and treatment (3+1=4). The residual error has 24 - (3+1) = **20 degrees of freedom**.
...
For benchmark, get the mean of all 24 observations. The mean for all the observations is 6.5846. There is only one level for benchmark so this number is placed into each of the cells for the benchmark factor in @fig-benchmark.

::: column-page
![Raw data and mean for benchmark factor](images/BF1_mean_benchmark.png){#fig-benchmark}
:::
...
::: column-page
![Raw data and means for benchmark and training factors](images/BF1_means_decomp.png){#fig-training-means}
:::
...
::: hoverchunk
[ df ]{.tooltipr}[A name you come up with for your dataset]{.tooltiprtext} [ \<- ]{.tooltipr}[The assignment operator. The result to the right of it gets stored in an object specified on the left]{.tooltiprtext} [ read_csv( ]{.tooltipr}[a command from the tidyverse to read in csv files]{.tooltiprtext} [ "data/toothpaste_BF2.csv" ]{.tooltipr}[The path to the csv file containing the data]{.tooltiprtext} [ ) ]{.tooltipr}[Functions always end with a closing parenthesis]{.tooltiprtext} <br/>[ plaque_aov]{.tooltipr}[A name you come up with for your model]{.tooltiprtext} [ \<- ]{.tooltipr}[The assignment operator. The result to the right of it gets stored in an object specified on the left]{.tooltiprtext} [ aov( ]{.tooltipr}[A function to define the model]{.tooltiprtext} [ Plaque ]{.tooltipr}[The response, or y, variable in the model. It is numeric.]{.tooltiprtext} [ \~ ]{.tooltipr}[The \~ is like an equal sign in the model. Items on the left of \~ represent y, on the right you define independent factors (i.e. x's).]{.tooltiprtext} [ Brush, ]{.tooltipr}[The independent variable containing the names for the 4 types of toothbrushes.]{.tooltiprtext} [ data = df ]{.tooltipr}[Tell the model to look in the dataset named "df" for Plaque and Brush variables]{.tooltiprtext} [ ) ]{.tooltipr}[Functions always end with a closing parenthesis]{.tooltiprtext} <br/>[ summary( ]{.tooltipr}[Give important information about an object. When called on an aov object the default is to print the ANOVA table]{.tooltiprtext} [ plaque_aov ]{.tooltipr}[ Whatever you named your ANOVA model in the previous line ]{.tooltiprtext} [ ) ]{.tooltipr}[ Functions always end with a closing parenthesis ]{.tooltiprtext}<br/>[ Click to view output ]{.tooltipr style="float:right;"}[Click to View Output.]{.tooltiprtext}
:::

</a>

::: {#bff1 style="display:none;"}
```{r message = FALSE}
