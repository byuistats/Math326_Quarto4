...
```{r echo = FALSE}
Source <- c("Grand Mean", "Brush", "Participant", "Residual Error", "Total")
df <- c(1,3,5, 15, 24)
SS <- rep("", 5)
MS <- rep("", 5)
Fvalue <- rep("", 5)
...
SS <- c(12437.43, 86.31, 18.27, 132.29, 12674.30)
...
To calculate a mean square (MS), simply divide a factor's sum of questions by its degrees of freedom. The mean square calculations are:
...
MS <- c(12437.43, 28.77, 3.65, 8.82, NA)
...
We will proceed with a test of the toothbrush type, represented as $\alpha$ in @eq-cb1. 

$$H_0: \alpha_i = 0 \text{ for all }i$$

$$H_a: \alpha_i \ne 0 \text{ for some }i$$

To test the hypothesis, we need to compare the mean square (MS) for toothbrush to the mean square for residual error (abbreviated as MSE). This ratio of variances is called an F statistic. The F statistical calculation is

$$
F_\text{Brush} = \frac{MS_\text{Brush}}{df_\text{Brush}} = \frac{28.77}{8.82} = 3.26
$$
...
Fvalue <- c(NA, 3.26, NA, NA, NA)
pvalue <- c(NA, 0.051, NA, NA, NA)
...
cb_aov <- aov(Y ~ treatment + block, data = YourDataSet)
summary(cb_aov)
```
...
cb_aov <- aov(Plaque ~ Brush + ParticipantID, data=cb1) #note the + instead of *
summary(cb_aov)
```
