...
In this section, we will do the analysis of variance by hand by decomposing the variation into its parts.  Th

Below is the factor structure with accompanying model for the Fungicide study.  We ignore randomization for simplicity.

![](images/Split_Plot_Factor_Decomposition.png){#fig-block-sort fig-align="left" width="950" height="220"}

Filled out with our example data, we get figure XX:

![](images/Split_Plot_Data_Decomposition.png){#fig-block-sort fig-align="center" width="950" height="450"}
<br>

The grand mean is, as usual, the overall average yield across all factor levels.  

The Fungicide effect is the average yield for each level of spray treatment minus the grand mean.  

The field effect is the average yield for each field minus the grand mean.

The variety effect is the average yield for each variety minus the grand mean.

The interaction effect is the average yield for each fungicide-by-variety combination minus the sum of the grand mean and the individual effects for fungicide and variety.  
