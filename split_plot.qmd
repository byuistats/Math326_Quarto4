...
The area under the $F_{1,2}$ distribution to the right of the F test statistic constitutes the p-value. In this case, it is **p=** **0.687**. With such a high p-value, we conclude there is insufficient evidence that fungicide effects yield. The following code will get you the p-value:

```{r}
#| eval: false

pf(.2175, 1, 2, lower.tail = FALSE)

```

This is a more generic representation of the code, using description of the input arguments rather than actual numbers from this particular problem:
...
The `numerator_df` is the degrees of freedom associated with the mean squares values used in the numerator of the F statistic calculation. The `denominator_df` is the degrees of freedom associated with the mean squares value used in the denominator of the F statistic. The `lower.tail` argument tells the function whether you want to calculate the area under the F distribution (i.e. the probability) in the lower tail or the upper tail. Since we are calculating a p-value, we want the area under the curve in the upper tail of the distribution. (The definition of a p-value is the probability of getting a test statistic **more extreme** than the one observed, assuming the null hypothesis is true).

Though we calculated the correct p-value manually, there is other R code, not covered in this book, that will calculate the correct F test by default.[^4]
