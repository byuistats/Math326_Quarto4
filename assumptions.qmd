...
Since we know the mean of the residual errors is zero, the second term goes to zero and the mean of observations is the best estimate of the true value. Of course, the more observations we average over the better our estimate of the truth will be.
...
In the F-statistic calculation the denominator is a pooled variance; or in other words, an average of the within factor level variances. When factor level sample sizes are unequal, then the average variance is biased toward the factor level with the largest sample size. Thus, it becomes important to show that the within variance estimate for each factor level is (relatively) equal when sample sizes are not equal. The greater disparity in sample sizes, the more important it is to have *within* factor levels variances that are similar. (Inflation of Type I error tends to be worse if the smallest group has the largest variance.) The F-distribution used to calculate p-values is based on the assumption of equal variances. When there is non-constant variance, the ratio of *between* group variance to *within* group variance no longer follows the F-distribution and the p-value calculations will be off.
...
-   Residual from the model[^2]
-   The value from the standard normal distribution (i.e. Z distribution) associated with the sample quantile.

[^2]: The name "Q-Q plot" (with 2 q's) comes from the fact you can plot the sample quantile of the residuals instead of plotting the residual itself: sample quantiles on the y-axis and theoretical quantiles on the x-axis. The plots we will be creating in R put the residual on the y-axis. This is simpler to interpret and to calculate because it removes the need to store the sample quantile values.
...
It can be difficult to tell if the line is straight enough. `qqPlot` from the `car` package (see @fig-QQPlot_brush2) adds boundary lines to help you determine if the points are out of bounds[^3]. If sections of the plot are out of bounds the assumption of normally distributed residuals can be considered violated.

[^3]: `qqPlot` also does a few other calculations differently, most notably it uses [studentized residuals](https://online.stat.psu.edu/stat462/node/247/) instead of standardized residuals.
...
It is also important to recognize that ANOVA is just one of many analysis tools available[^4]. Kruskal-Wallis is one example. These alternative test assumptions are less stringent. But if its requirements are met, ANOVA offers greater statistical power. It can also be difficult to extend some of these alternative tests to more complex designs.
...
One other approach is to emphasize the tests of contrasts and comparisons rather than omnibus tests. These more specific tests can handle non-constant variances and non-normal residuals. In fact, many researchers skip the omnibus ANOVA F-test and will go directly to testing the specific comparisons that motivated the experiment in the first place.
